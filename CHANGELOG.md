...
- [修复] 复制节点时同步保留其引用连线，单独复制节点后仍连接原引用节点。

- [调整] 提高画布 Ctrl + 滚轮缩放的灵敏度。
+ [调整] 画布资产和图片节点统一限制显示尺寸，最大不超过 420×300 并保持原始比例。

+ [调整] 画布新建节点默认尺寸整体放大，普通节点调整为 420×300。

+ [调整] 画布资产支持直接拖动矩形框选，选中后可批量加入画布或删除素材库资产。

+ [新增] 资产导入支持人物、场景、物品、音频分类，资产面板可按分类筛选并全选当前分类。

+ [新增] 画布资产面板支持多选、全选当前筛选结果并批量加入画布，自动网格排列且避开已有节点。

+ [调整] 结果节点再次生成时原位更新并保留连线，新增节点生成历史入口，可查看之前的图片、视频、音频和文本内容。

+ [修复] 点击视频播放控件后返回画布，空格不再误触发播放或暂停，而是正常临时切换抓手。

+ [新增] 点击画布引用连线显示小剪刀，再次点击剪刀即可解除引用并保留两端节点。

+ [调整] 画布普通滚轮改为上下平移，Shift + 滚轮左右平移，仅 Ctrl + 滚轮缩放，并同步更新快捷键说明。

+ [修复] 画布页面的 Ctrl + 滚轮在工具图标、节点和提示词区域统一缩放画布，阻止浏览器整页缩放，保持提示词面板的显示尺寸不变。

+ [调整] 画布默认使用鼠标选择，非编辑状态按住空格临时切换抓手，抓手模式可从节点和提示词区域拖动画布且不会触发编辑。

+ [新增] 视频调用脚本支持通过 `@videoSettings` 声明按渠道、模型生效的清晰度、比例、时长和模式，设置面板与脚本入参同步使用。

+ [修复] 节点提示词输入框支持用 `@` 选择未连接的画布素材，自动添加参考连线并插入对应引用，展开编辑器同步支持。

+ [新增] 多选节点后可从虚线外框右侧中点拖出批量连线，连接已有节点或创建一个共同连接的新节点。
