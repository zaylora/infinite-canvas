...
import { Download, Ellipsis, FolderPlus, History, Image as ImageIcon, Info, MessageSquare, Minus, Music2, Plus, RefreshCw, Settings2, Trash2, Ungroup, Upload, Video } from "lucide-react";
...
type CanvasNodeHoverToolbarProps = {
    ...
    onHistory: (node: CanvasNodeData) => void;
...
export function CanvasNodeHoverToolbar({
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(isImage || isVideo || isAudio || isText ? [{ id: "generationHistory", title: "查看之前生成的内容", label: "生成历史", icon: <History className="size-4" />, onClick: () => onHistory(node) }] : []),
    ];
    const toolbarTools = hasImage ? [...baseToolbarTools, ...nodeToolbarTools].filter((tool) => tool.id === "generationHistory" || quickImageToolIdSet.has(tool.id as ImageQuickToolId)) : [...baseToolbarTools, ...nodeToolbarTools, ...extraTools];
    const selectableImageToolbarTools = [...baseToolbarTools, ...nodeToolbarTools].filter((tool) => tool.id !== "retry" && tool.id !== "generationHistory") as ImageToolbarSettingsTool[];
