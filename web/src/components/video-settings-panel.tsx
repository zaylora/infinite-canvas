...
import { applyVideoScriptSettings, resolveVideoScriptSettings } from "@/lib/video-script-settings";
...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: VideoSettingsPanelProps) {
    ...
    const settings = resolveVideoScriptSettings(config);
    config = applyVideoScriptSettings(config, settings);
    const range = settings?.seconds || videoSecondsRange;
    const seconds = Number(settings?.seconds ? config.videoSeconds : clampVideoSeconds(config.videoSeconds || "6"));
    const resolutions = settings?.resolution?.map((value) => ({ value, label: `${value}p` })) || resolutionOptions;
    const ratios = settings?.ratio?.map((value) => videoRatioOptions.find((item) => item.value === value)!) || videoRatioOptions;
    const modes = settings?.mode ? videoModeOptions.filter((item) => settings.mode!.includes(item.value)) : videoModeOptions;
            ...
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <SettingGroup title={t("settingsPanels.video.quality")} color={theme.node.muted}>
                    <div className="grid grid-cols-4 gap-2.5">
                        ...
                        {!settings?.resolution ? <ResolutionInput value={resolution} theme={theme} onChange={selectResolution} /> : null}
                    </div>
                </SettingGroup>
                <SettingGroup title={t("settingsPanels.video.size")} color={theme.node.muted}>
                    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2.5">
                        <DimensionInput prefix="W" value={dimensions.width} disabled={selectedRatio === "auto" || Boolean(settings?.resolution || settings?.ratio)} theme={theme} onChange={(value) => updateDimension("width", value, dimensions, onConfigChange)} />
                        <span className="text-lg opacity-45">↔</span>
                        <DimensionInput prefix="H" value={dimensions.height} disabled={selectedRatio === "auto" || Boolean(settings?.resolution || settings?.ratio)} theme={theme} onChange={(value) => updateDimension("height", value, dimensions, onConfigChange)} />
                    </div>
                </SettingGroup>
                ...
                <SettingGroup title={t("settingsPanels.video.seconds")} color={theme.node.muted}>
                    <div className="flex items-center gap-3" onMouseDown={(event) => event.stopPropagation()}>
                        <Slider className="min-w-0 flex-1" min={range.min} max={range.max} disabled={range.min === range.max} step={1} value={seconds} onChange={(value) => onConfigChange("videoSeconds", String(Array.isArray(value) ? value[0] : value))} />
                        <SecondsInput value={seconds} min={range.min} max={range.max} theme={theme} onCommit={(value) => onConfigChange("videoSeconds", String(value))} />
